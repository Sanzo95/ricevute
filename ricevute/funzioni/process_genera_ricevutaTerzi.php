...
$cognome = sostituisci($_POST['cognome']);
$nome = sostituisci($_POST['nome']);
...
//è l'id della ricevuta che si sta emettendo
...
if ($insert_query_ricevute = $mysqli->prepare("INSERT INTO ricevute (ricevuteEmesse, codiceSocioEmittente, nome, cognome,
  mailDestinatario, dataEmissione, annoAssociativo, importo, causale, file)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    $insert_query_ricevute->bind_param('iissssidss', $ricevuteEmesse, $_SESSION['cod'], $nome, $cognome,
    $mailDestinatario, $dataEmissione, $annoAssociativo, $importo, $causale, $file);
